...
import static paw.old.PawConstants.*;
import static paw.old.greycat.tasks.VocabularyTasks.VOCABULARY_MAP;
import static paw.old.greycat.tasks.VocabularyTasks.retrieveVocabularyNode;
    ...
    public static Task updateOrCreateTokenizeRelationFromString(String tokenizerVar, String nodesVar, String content, String relationName) {
        return newTask()
                ...
                .thenDo(ctx -> {
                    ...
                    ctx.setVariable("longPosition", tokenizedString.get_numberPosition());
                    ctx.continueTask();
                })
                .readVar(nodesVar)
                .forEach(
    ...
    private static Task createTokenRelation(String tokenizerVar, String relationName) {
        return newTask()
                .thenDo((TaskContext ctx) -> {
                            ...
                            Map<Integer, Integer> intPosition = (Map<Integer, Integer>) ctx.variable("longPosition").get(0);
                            ...
                            IntIntMap longs = (IntIntMap) node.getOrCreate(TOKENIZE_CONTENT_INTEGER, Type.INT_TO_INT_MAP);
                            if (intPosition != null) {
                                for (Map.Entry<Integer, Integer> integer : intPosition.entrySet()) {
                                    longs.put(integer.getKey(), integer.getValue());
                                }
                            }
